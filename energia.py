import streamlit as st
import pandas as pd
import plotly.express as px

@st.cache()
def load_energy_data(data_init,data_end):
    renewal_ratio_nan = 0.5
    df = pd.read_csv('energy_data.csv', index_col=0, parse_dates=True,dtype='float64')
    df = df.loc[data_init:data_end]
    df['renewal_energy_purchased'].fillna(df['electric_energy_purchased']*renewal_ratio_nan,inplace=True)
    df['Não-renovável fornecida/kWh'] = df['electric_energy_purchased'] - df['renewal_energy_purchased']
    df = df.drop(columns = ['electric_energy_purchased'])
    df = df.rename(columns = {
        'renewal_energy_purchased': 'Renovável fornecida/kWh',
        'renewal_energy_produced': 'Renovável produzida/kWh'
    })
    return df


def energy_global_pie(data_init,data_end,is_table=False,is_plot=True):
    df = load_energy_data(data_init,data_end)
    df_sum = pd.DataFrame()
    column_name = 'Energia/kWh'
    title = f'Balanço energetico de {data_init} a {data_end}'
    df_sum[column_name] = df.sum()
    if (df.size > 0): 
        if is_plot:
            fig = px.pie(df_sum,values=column_name,names=df_sum.index,title=title,color=df_sum.index)
            st.plotly_chart(fig)
        if is_table:
            st.dataframe(df_sum.style.format("{:.2f}"))
    else: 
        st.write("Periodo sem dados.")

def energy_global(data_init,data_end,is_table,is_plot):
    df = load_energy_data(data_init,data_end)
    title = f'Balanço energetico de {data_init} a {data_end}'
    if (df.size > 0):
        if is_plot:
            fig = px.area(df,title=title)
            st.plotly_chart(fig)
        if is_table:
            st.dataframe(df.style.format("{:.2f}"))
    else: 
        st.write("Periodo sem dados.")
